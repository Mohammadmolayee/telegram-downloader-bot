...
def _run_download_sync(url, outdir, prefer_quality="480"):
    opts = {
        "format": "bestvideo[ext=mp4][height<=?{q}]+bestaudio[ext=m4a]/best[height<=?{q}]/bestaudio/best".format(q=int(prefer_quality)),
...
async def _download_task(user_id, url, bot, reply_message, dl_id, prefer_quality="480"):
    outdir = f"temp/{user_id}"
    try:
        info = await asyncio.to_thread(_run_download_sync, url, outdir, prefer_quality)
        ...
        file_path = files[0]
        # send with caption and watermark
        caption = f"Downloaded by @professional_dawnloder_bot\n{info.get('title','')}"
        await bot.send_document(chat_id=user_id, document=InputFile(file_path), caption=caption)
        set_download_status(dl_id, "done", os.path.basename(file_path))
        try: await reply_message.edit_text("✔ دانلود تمام شد.")
        except: pass
        ...
        try:
            ...
            if os.path.isdir(outdir): os.rmdir(outdir)
        except Exception:
            pass
        RUNNING.pop(user_id, None)

async def start_download(user_id, url, bot, reply_message, prefer_quality="480"):
    if user_id in RUNNING:
        await reply_message.edit_text("⚠️ شما یک دانلود فعال دارید. ابتدا لغو کن.")
        return None
    platform = detect_platform(url)
    dl_id = add_download(user_id, url, platform, "", "running")
    task = asyncio.create_task(_download_task(user_id, url, bot, reply_message, dl_id, prefer_quality))
